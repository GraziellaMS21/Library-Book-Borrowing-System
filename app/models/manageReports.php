...
class Reports extends Database
{
    ...
    {
        $sql = "SELECT b.book_title, b.author, COUNT(bd.bookID) AS borrow_count
                FROM borrowing_details bd
                JOIN books b ON bd.bookID = b.bookID
                GROUP BY bd.bookID, b.book_title, b.author
    ...
    {
        $sql = "SELECT 
                    u.college_department,
                    COUNT(bd.borrowID) AS total_borrows
                FROM borrowing_details bd
                JOIN users u ON bd.userID = u.userID
                WHERE u.college_department IS NOT NULL AND u.college_department != ''
                GROUP BY u.college_department
    ...
    public function getMonthlyFineCollectionTrend()
    {
        $sql = "SELECT 
                ...
                LEFT JOIN (
                    SELECT DATE_FORMAT(return_date, '%M') AS month, SUM(fine_amount) AS total_fines
                    FROM borrowing_details
                    WHERE fine_status = 'Paid'
                    GROUP BY DATE_FORMAT(return_date, '%M')
                ) AS f_paid ON Months.month = f_paid.month
                LEFT JOIN (
                    SELECT DATE_FORMAT(expected_return_date, '%M') AS month, SUM(fine_amount) AS total_fines
                    FROM borrowing_details
                    WHERE fine_status = 'Unpaid'
                    GROUP BY DATE_FORMAT(expected_return_date, '%M')
                ) AS f_unpaid ON Months.month = f_unpaid.month
                GROUP BY Months.month
                ORDER BY STR_TO_DATE(CONCAT('01 ', Months.month, ' 2000'), '%d %M %Y') ASC";
    ...
    public function getTopUnpaidFinesUsers($limit = 5)
    {
        $sql = "SELECT 
                    ...
                    COUNT(bd.borrowID) AS overdue_items
                FROM borrowing_details bd
                JOIN users u ON bd.userID = u.userID
    ...
    {
        $sql = "SELECT 
                    DATE_FORMAT(date_registered, '%M') AS month,
    ...
    public function getBooksPerCategory()
    {
        $sql = "SELECT 
                    c.category_name,
                    SUM(b.book_copies) AS total_copies
                FROM books b
    ...
    public function getAverageBorrowDurationByCategory()
    {
        $sql = "SELECT 
                    c.category_name,
                    AVG(DATEDIFF(bd.return_date, bd.pickup_date)) AS avg_duration_days
                FROM borrowing_details bd
                JOIN books b ON bd.bookID = b.bookID
                JOIN category c ON b.categoryID = c.categoryID
    ...
    {
        $sql = "SELECT 
                    DATE_FORMAT(return_date, '%M') AS month,
                    COUNT(borrowID) AS late_returns
                FROM borrowing_details
                WHERE borrow_status = 'Returned'
    ...
    public function getBorrowerTypeBreakdown()
    {
        $sql = "SELECT 
                    ut.type_name, 
                    COUNT(bd.borrowID) AS borrow_count
                FROM borrowing_details bd
                JOIN users u ON bd.userID = u.userID
    ...
    {
        $sql = "SELECT b.author, COUNT(bd.bookID) AS borrow_count
                FROM borrowing_details bd
                JOIN books b ON bd.bookID = b.bookID
