...
if ($action === 'register' && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- VALIDATION START ---
    if (empty($register["userTypeID"])) $errors["userTypeID"] = "Please Choose From the Following";
    if (empty($register["lName"])) $errors["lName"] = "Last Name is required";
    if (empty($register["fName"])) $errors["fName"] = "First Name is required";

    if ($register["userTypeID"] == 1 || $register["userTypeID"] == 2) {
        if (empty($register["id_number"])) $errors["id_number"] = "ID Number is required";
        elseif (!is_numeric($register["id_number"])) $errors["id_number"] = "ID Number Format is Invalid";
        if (empty($register["departmentID"])) $errors["departmentID"] = "College/Department is required";
    }

    if (empty($register["contact_no"])) $errors["contact_no"] = "Contact Number is required";
    elseif (!is_numeric($register["contact_no"]) || strlen($register["contact_no"]) != 11) $errors["contact_no"] = "Contact Number Format is Invalid";

    if (empty($register["email"])) $errors["email"] = "Email is required";
    else if ($registerObj->isEmailExist($register["email"])) $errors["email"] = "Email already exist";

    if (empty($register["password"])) $errors["password"] = "Password is required";
    if (empty($register["conPass"])) $errors["conPass"] = "Please Confirm Your Password";
    else if ($register["password"] !== $register["conPass"]) $errors["conPass"] = "Passwords do not match";

    if (empty($register["agreement"])) $errors["agreement"] = "You must Agree to the Terms and Conditions";
    ...
    if (empty(array_filter($errors))) {
        if (move_uploaded_file($_FILES["imageID"]["tmp_name"], $register["imageID_dir"])) {
            ...
            if ($registerObj->addUser()) {
                ...
                if (file_exists($register["imageID_dir"])) unlink($register["imageID_dir"]);
