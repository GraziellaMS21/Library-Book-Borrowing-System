...
// Borrowing restrictions 
// Removed local calculateMaxCopiesAllowed function in favor of Model method
// STRICT CHECKS
if ($user['account_status'] !== 'Active') {
    $errors['account_blocked'] = "Your account is currently {$user['account_status']}. Borrowing is disabled.";
}

if ($borrowObj->checkAndApplyFines($userID)) {
    $errors['unpaid_fines'] = "You have unpaid fines. Please settle them before borrowing.";
}
...
if ($is_list_checkout) {
    ...
    if (empty($list_items_post)) {
        ...
        foreach ($list_items_post as $listID => $item) {
            ...
            $max_copies_allowed = $borrowObj->calculateMaxCopiesAllowed($userTypeID, $borrow_limit, $current_borrowed_count, $max_available, $is_borrowed);
