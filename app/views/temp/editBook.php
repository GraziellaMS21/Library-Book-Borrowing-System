...
if (isset($_GET['id']) && empty($book)) {
    // Only fetch from DB if there’s no old session data
    $bookID = $_GET['id'] ?? NULL;
            ...
            <div class="booksSection" id="bookSection">
                ...
                <div class="section manage_books h-full">
                    <div class="addBook rounded-xl p-4 bg-red-800 inline-block text-white my-2">
                        <button><a href="../../../app/views/librarian/booksSection.php"
                                class="bg-red-800 text-white px-4 py-2 rounded-lg hover:bg-red-700">
                                ← Back
                            </a></button>
                    </div>
                    <form action=" ../../../app/controllers/bookController.php?action=edit&id=<?= $bookID ?>"
                        method="POST">
