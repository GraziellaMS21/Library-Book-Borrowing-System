...
// Determines the current Modal to open automatically on error
...
// Load User Data for Modals (Edit, Delete, Return, Paid, View)
...
// Calculate Fines on the fly for display
        ...
        <div class="container">
            <div class="section manage_users h-full">
                ...
                <h2 class="text-2xl font-semibold mb-4 text-gray-700">
                    <?php
                    switch ($current_tab) {
                        case 'pending': echo 'Pending Request'; break;
                        case 'approved': echo 'Approved Request'; break;
                        case 'borrowed': echo 'Currently Borrowed'; break;
                        case 'unpaid': echo 'Fined'; break;
                            ...
                            foreach ($borrow_details as $detail) {
                                ...
                                <tr>
                                    ...
                                    <?php elseif ($current_tab == 'borrowed'): ?>
                                        <td><?= $detail["no_of_copies"] ?></td>
                                        <td><?= $detail["expected_return_date"] ?></td>
                                        <td class="font-semibold <?= ($detail["calculated_fine"] > 0) ? 'text-red-700' : 'text-gray-700' ?>">
                                        ...
                                        <td class="action text-center">
                                            <?php if ($detail["calculated_fine"] > 0) { ?>
                                                ...
                                                <button class="actionBtn bg-yellow-600 hover:bg-yellow-700 text-sm inline-block mb-1 cursor-pointer open-modal-btn"
        ...
        <form action="../../../app/controllers/borrowDetailsController.php" method="POST">
            ...
            <div class="bg-gray-100 p-4 rounded mb-4 text-sm">
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Book damaged/unavailable" class="mr-2"> Book unavailable / Damaged
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="User has overdue books" class="mr-2"> User has overdue books
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="User exceeded borrow limit" class="mr-2"> Exceeded borrow limit
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Policy Violation" class="mr-2"> Policy Violation
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Reserved for maintenance" class="mr-2"> Reserved for maintenance
                </label>
            </div>
...
<div id="cancelRequestModal" class="modal">
    <div class="modal-content max-w-md">
        ...
        <form action="../../../app/controllers/borrowDetailsController.php" method="POST">
            ...
            <div class="bg-gray-100 p-4 rounded mb-4 text-sm">
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="User requested cancellation" class="mr-2"> User requested cancellation
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Book not found on shelf" class="mr-2"> Book not found on shelf
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="System Error" class="mr-2"> System Error / Duplication
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Administrative Decision" class="mr-2"> Administrative Decision
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Unclaimed by deadline" class="mr-2"> Unclaimed by deadline
                </label>
            </div>
...
<div id="blockUserModal" class="modal <?= $open_modal == 'blockUserModal' ? 'open' : '' ?>">
    <div class="modal-content max-w-md">
        ...
        <form action="../../../app/controllers/borrowDetailsController.php" method="POST">
            ...
            <p class="mb-4 text-gray-700">
                Select reason for blocking <span class="font-bold user-name-span"><?= $modal_borrow_details['fName'] ?? '' ?></span>:
            </p>
            ...
            <div class="bg-gray-100 p-4 rounded mb-4 text-sm">
                ...
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Repeated Overdue Returns" class="mr-2"> Repeated Overdue Returns
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Lost/Damaged Books" class="mr-2"> Lost/Damaged Books
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Violation of Code of Conduct" class="mr-2"> Violation of Code of Conduct
                </label>
                <label class="flex items-center mb-2 cursor-pointer">
                    <input type="checkbox" name="reason_presets[]" value="Identity Theft / Fraud" class="mr-2"> Identity Theft / Fraud
                </label>
            </div>
...
<div id="returnBookModal" class="modal <?= $open_modal == 'returnBookModal' ? 'open' : '' ?>">
    <div class="modal-content max-w-md">
        ...
        <form id="returnBookForm" action="../../../app/controllers/borrowDetailsController.php?action=return&id=<?= $borrow_id ?? '' ?>" method="POST">
            ...
            <div class="input mb-4 p-3 bg-gray-100 rounded-lg">
                <label class="block font-semibold mb-1" for="original_condition_display">Original Condition (upon loan):</label>
            ...
            <div class="input">
                ...
                <select name="returned_condition" id="returned_condition" class="input-field w-full p-2 border rounded-lg focus:ring-red-800 focus:border-red-800">
                    <?php
    ...
    <div class="modal-content">
        ...
        <form id="editBorrowDetailForm" action="../../../app/controllers/borrowDetailsController.php?action=edit&id=<?= $borrow_id ?>" method="POST">
            ...
            <div>
                ...
                <p class="text-lg"><?= htmlspecialchars($modal_borrow_details['fName'] ?? 'N/A') . ' ' . htmlspecialchars($modal_borrow_details['lName'] ?? '') ?></p>
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="input">
                    ...
                    <input type="number" class="input-field !w-5/6" name="no_of_copies" value="<?= $modal_borrow_details["no_of_copies"] ?? "1" ?>">
                </div>
                <div class="input">
                    <label for="request_date">Borrow/Request Date<span>*</span> : </label>
                    <input type="date" class="input-field !w-5/6" name="request_date" value="<?= $modal_borrow_details["request_date"] ?? "" ?>">
                </div>
                <div class="input">
                    <label for="pickup_date">Pickup Date: </label>
                    <input type="date" class="input-field !w-5/6" name="pickup_date" value="<?= $modal_borrow_details["pickup_date"] ?? "" ?>">
                </div>
                <div class="input">
                    <label for="expected_return_date">Exp. Return Date<span>*</span> : </label>
                    <input type="date" class="input-field !w-5/6" name="expected_return_date" value="<?= $modal_borrow_details["expected_return_date"] ?? "" ?>">
                    <p class="text-sm text-gray-500 italic">Fine recalculation is automatic when fine is set to 0.00. <button id="fineReset" type="button" class="font-bold underline">Reset Fine</button></p>
                </div>
                <div class="input">
                    <label for="return_date">Actual Return Date: </label>
                    <input type="date" class="input-field !w-5/6" name="return_date" value="<?= $modal_borrow_details["return_date"] ?? "" ?>">
                </div>
                <div class="input">
                ...
                <div class="input">
                    ...
                    <input type="number" step="0.01" class="input-field !w-5/6" name="fine_amount" id="edit_fine_amount" value="<?= $modal_borrow_details["fine_amount"] ?? "0.00" ?>">
                </div>
                <div class="input">
            ...
            <div class="cancelConfirmBtns">
                <button type="button" data-modal="editBorrowDetailModal" data-tab="<?= $current_tab ?>" class="close bg-gray-300 text-gray-800 px-4 py-2 rounded-lg font-semibold hover:bg-gray-400 cursor-pointer">Cancel</button>
...
<div id="deleteConfirmModal" class="modal delete-modal <?= $open_modal == 'deleteConfirmModal' ? 'open' : '' ?>">
    <div class="modal-content max-w-sm">
        ...
        <p class="mb-6 text-gray-700">
            ...
            <span class="font-semibold italic"><?= $modal_borrow_details['fName'] ?? 'N/A' . ' ' . $modal_borrow_details['lName'] ?? '' ?></span>
            (Borrow ID: <span class="font-semibold"><?= $modal_borrow_details['borrowID'] ?? $borrow_id ?></span>)?
            This action cannot be undone.
        </p>
        <div class="cancelConfirmBtns">
            <button type="button" data-modal="deleteConfirmModal" class="close bg-gray-300 text-gray-800 px-4 py-2 rounded-lg font-semibold hover:bg-gray-400">Cancel</button>
            <a href="../../../app/controllers/borrowDetailsController.php?action=delete&id=<?= $modal_borrow_details['borrowID'] ?? $borrow_id ?>" class="text-white px-4 py-2 rounded-lg font-semibold cursor-pointer bg-red-600 hover:bg-red-700">Confirm Delete</a>
        </div>
    </div>
</div>
...
<div id="paidConfirmModal" class="modal delete-modal <?= $open_modal == 'paidConfirmModal' ? 'open' : '' ?>">
    <div class="modal-content max-w-lg">
        ...
        <form action="../../../app/controllers/borrowDetailsController.php?action=paid&id=<?= $borrow_id ?? '' ?>" method="POST">
            ...
            <p class="mb-4 text-gray-700">Mark fine as <strong>Paid</strong> and book as <strong>Returned</strong> for: <span class="font-semibold text-red-800"><?= $modal_borrow_details['book_title'] ?? 'N/A' ?></span>.</p>
            
            <label class="block font-semibold mb-1" for="returned_condition">Returned Condition<span>*</span> :</label>
            <select name="returned_condition" class="input-field w-full p-2 border rounded-lg focus:ring-red-800 focus:border-red-800">
                <option value="">---Select Condition---</option>
                <?php foreach ($condition_options as $option) {
                     $selected = (($modal_borrow_details['returned_condition'] ?? '') == $option) ? 'selected' : '';
                     echo "<option value='{$option}' {$selected}>{$option}</option>";
            ...
            <div class="cancelConfirmBtns">
                <button type="button" class="close bg-gray-300 text-gray-800 px-4 py-2 rounded-lg font-semibold hover:bg-gray-400" data-modal="paidConfirmModal" data-tab="<?= $current_tab ?>">Cancel</button>
